...
│
├── utils/
│   ├── metrics.py             Accuracy, Precision, Recall, F1, AUC + MetricTracker
...
│   └── visualization.py       Skeleton overlay, bounding boxes, evaluation plots
│
...
### `metrics.py`
Centralized evaluation metrics used by `evaluate.py` and `train.py`.
...
# Returns: accuracy, precision, recall, f1, roc_auc, avg_precision

# Accumulate metrics across batches during training
tracker = MetricTracker()
tracker.update(probs, labels, loss.item())   # call each batch
metrics = tracker.compute()                  # call after epoch
tracker.reset()
```
...
### `visualization.py`
Two groups of functions — real-time drawing and evaluation plots.

**Real-time:**
```python
from utils.visualization import (
    draw_pose_skeleton, draw_person_box,
    draw_fps_counter, draw_info_panel, FPSCounter
)

fps_counter = FPSCounter(window=30)
fps = fps_counter.tick()

draw_pose_skeleton(frame, pose_features, bbox, is_violent=True)
draw_person_box(frame, bbox, track_id, violence_score, is_violent, is_ready)
draw_fps_counter(frame, fps)
draw_info_panel(frame, num_persons=2, violence_detected=True)
```

**Skeleton color scheme (non-violent):**
...
| Left leg | Green |
| Right leg | Blue |
| Violent person | All Red |

**Evaluation plots:**
```python
from utils.visualization import (
    plot_confusion_matrix, plot_roc_curve,
    plot_precision_recall_curve, plot_training_history
)

plot_confusion_matrix(cm, ["Non-Violence", "Violence"], save_path="outputs/cm.png")
plot_roc_curve(fpr, tpr, auc_score, save_path="outputs/roc.png")
plot_precision_recall_curve(precision, recall, ap, save_path="outputs/pr.png")
plot_training_history(train_losses, val_losses, train_accs, val_accs, save_path="outputs/history.png")
```
