...
NEW FEATURES:
  ...
  [ALARM]      When ANY tracked person is flagged violent, an audible
               alarm sound plays automatically (on a background thread,
               so it never blocks video processing). Uses the same
               cooldown pattern as screenshots to avoid a continuous
               siren during a sustained violent event.
...
def play_alarm_sound():
    ...
    try:
        if platform.system() == "Windows":
            ...
            # any extra dependency. Less attention-grabbing than a real
            # tone, but requires zero additional setup.
            for _ in range(3):
...
def main(args):
    ...
    # [ALARM] Same global-cooldown pattern as screenshots, so a sustained
    # violent event triggers periodic alerts rather than a continuous siren.
    last_alarm = 0.0
    ...
    while True:
        ...
        # ── [ALARM] One alert per cooldown window if ANYONE is violent ────────
        # Runs on a background thread (play_alarm_async) so the beep/tone
        # never blocks the next frame from being read and processed —
        # critical since lag was already a concern in multi-person mode.
        if any_violence_this_frame and not args.disable_alarm:
            now = time.time()
